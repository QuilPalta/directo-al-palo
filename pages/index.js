    ...
    <div className="min-h-screen bg-[#0a0a0a] text-white font-sans">
      <header className="py-10 border-b border-yellow-600/20 bg-black flex flex-col items-center">
        ...
        <nav className="mt-6 flex gap-6 text-xs font-bold uppercase tracking-widest text-gray-400">
          <span className="hover:text-yellow-500 cursor-pointer">Fútbol</span>
          <span className="hover:text-yellow-500 cursor-pointer">WWE</span>
          <span className="hover:text-yellow-500 cursor-pointer">Tenis</span>
          <Link href="/colaboradores" className="text-yellow-600 border border-yellow-600 px-3 py-1 rounded hover:bg-yellow-600 hover:text-black transition">Redacción</Link>
        </nav>
      </header>

      <main className="max-w-6xl mx-auto p-6 md:p-12">
        {cargando ? (
          <div className="flex justify-center items-center h-64 text-yellow-500 font-black animate-pulse uppercase tracking-widest">
            Buscando últimas jugadas...
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
            {noticias.map((nota) => (
              /* Enlace dinámico usando el ID de la noticia */
              <Link href={`/noticia/${nota.id}`} key={nota.id}>
                <article className="flex flex-col group cursor-pointer h-full">
                  <div className="relative overflow-hidden aspect-video mb-4 border-b-4 border-yellow-600 bg-zinc-900 shadow-xl">
                    ...
                    />
                    <span className="absolute top-2 left-2 bg-yellow-600 text-black text-[10px] font-black px-2 py-1 uppercase tracking-tighter">
                      {nota.categoria}
                    </span>
                  </div>
                  <h2 className="text-2xl font-bold leading-tight mb-3 group-hover:text-yellow-500 transition-colors uppercase italic">
                    {nota.titulo}
                  </h2>
        ...
        {!cargando && noticias.length === 0 && (
          <div className="text-center py-20 border-2 border-dashed border-zinc-900 rounded-lg">
            <p className="text-zinc-600 font-bold uppercase tracking-[0.3em]">No hay noticias publicadas.</p>
          </div>
        )}
      </main>
